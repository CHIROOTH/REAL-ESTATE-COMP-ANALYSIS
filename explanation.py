...
def generate_pdf(report: dict, input_data: dict, out_path="results/appraisal_report.pdf"):
...
SYSTEM = """You are a real estate valuation analyst. Use ONLY provided data. Return ONLY valid JSON — no markdown, no preamble.
Output: {"estimated_value":0,"model_used":"","confidence_label":"High|Medium|Low","summary":"","valuation_logic":"",
...
def call_ollama(input_data: dict) -> dict:
...
if __name__ == "__main__":
    ...
    input_data = {
        "query_index": 77, "actual_price": 649900,
        "global_prediction": 773421.18,
        "local_predictions": {"Linear Regression": 1176440, "Ridge Regression": 596360, "Random Forest": 599181, "XGBoost Local": 416787},
        "query_features": {"Bedrooms": 2, "Bathrooms": 2, "Square Footage": 1164, "Acreage": 0.0, "Garage": 1, "Parking": 0, "Fireplace": 1, "Latitude": 50.27, "Longitude": -119.26},
        "similar_properties": [{"price": 639000, "features": {"Bedrooms": 2, "Bathrooms": 2, "Square Footage": 1154}, "distances": {"location_dist_km": 5.15, "feature_dist_norm": 0.0008, "total_distance": 0.0037}}],
    }

    input_data = add_influence_weights(input_data)
    report = call_ollama(input_data)
    generate_pdf(report, input_data, "results/appraisal_report.pdf")